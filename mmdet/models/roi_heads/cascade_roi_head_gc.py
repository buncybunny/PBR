        ...
        for i in range(self.num_stages):
            ...
            if i < self.num_stages - 1:
                ...
                with torch.no_grad():
                    ...
                    # # # Mein coden
                    # # roi_labels_without_gt (512-k)에서 roi_label이 Novel인 애들 뽑기
                    #
                    # if img_metas[0]['filename'].split('/')[1] == 'VOCdevkit':
                    ...
                    #
                    # else:
                    #     # novel class로 분류된 predicted bboxes 중에
                    #     novel_proposals_inds = [i for i in range(len(wo_gt_roi_labels)) if
                    #                             wo_gt_roi_labels[i] in range(60, 80)]
                    # non_empty = None
                    # empty = None
                    # for n, inds in enumerate(novel_proposals_inds):
                    #     if any(inds):
                    #         non_empty = n
                    #     else:
                    #         empty = n
                    #
                    # if non_empty != None:
                    #     # print('hey', augmented_img_features)
                    #     if any(novel_proposals_inds):
                    #         # novel_proposals_coors = [proposals[novel_proposals_ind] for proposals, novel_proposals_ind in
                    #         #                          zip(proposal_list, novel_proposals_inds)]
                    #         for p in range(len(proposal_list)):
                    #             if any(novel_proposals_inds[p]):
                    #                 novel_proposals_coors.append((proposal_list[p])[novel_proposals_inds[p]])
                    #             else:
                    #                 novel_proposals_coors.append([])
                    #         # for proposals, novel_proposals_ind in zip(proposal_list, novel_proposals_inds):
                    #         #     if any(novel_proposals_ind):
                    #         #         novel_proposals_coors.append(proposals[novel_proposals_ind])
                    #         #     else:
                    #         #         novel_proposals_coors.append([])
                    #         # GT bboxes중에서 novel objects인 애들 고르기
                    #         selected_gt_novel_bboxes = []
                    #         ious = []
                    #         if img_metas[0]['filename'].split('/')[1] == 'VOCdevkit':
                    #             for gt_bbox, gt_label in zip(gt_bboxes, gt_labels):
                    #                 selected_gt_novel_bboxes.append(
                    #                     gt_bbox[[l for l, label in enumerate(gt_label) if label in range(15, 20)]])
                    #         else:
                    #             for gt_bbox, gt_label in zip(gt_bboxes, gt_labels):
                    #                 selected_gt_novel_bboxes.append(
                    #                     gt_bbox[[l for l, label in enumerate(gt_label) if label in range(60, 80)]])
                    #         # GT bboxes와의 iou 계산
                    #         # for novel_proposals_coor, selected_gt_novel_bbox in zip(novel_proposals_coors,
                    #         #                                                         selected_gt_novel_bboxes):
                    #         #     if novel_proposals_coor.shape[0] != 0:
                    #         #         ious.append(bbox_overlaps(novel_proposals_coor, selected_gt_novel_bbox, mode='iou',
                    #         #                                   is_aligned=False, eps=1e-6))
                    #         #         # multiple novel objects in a single image
                    #         #         if ious[-1].shape[1] != 1:
                    #         #             print(ious[-1].shape[1])
                    #         #         else:
                    #         #             print(ious[-1].shape[1])
                    #         #     else:
                    #         #         ious.append([])
                    #         # total augmentation per image == num_aug*batch (e.g. 3x2)
                    #         num_aug = 5
                    #         # CJ, GB
                    #         augmented_imgs = self.aug_imgs(selected_gt_novel_bboxes, img, img_metas, num_aug)
                    #
                    #         # TODO: batch!=2 case
                    #         # batch1과 batch2과 cross로 들어
                    #         aug_imgs = []
                    #         if augmented_imgs[0].numel() and augmented_imgs[1].numel():
                    #             for aug1, aug2 in zip(augmented_imgs[0], augmented_imgs[1]):
                    #                 aug_imgs.append(torch.stack((aug1, aug2), ))
                    #         else:
                    #             for a in range(len(augmented_imgs[non_empty])):
                    #                 aug_imgs.append(augmented_imgs[non_empty][a].unsqueeze(0))
                    #
                    #         augmented_img_features = [neck(backbone(augmented_img)) if
                    #                                   augmented_img.shape[0] != 0 else torch.tensor([]) for
                    #                                   augmented_img in
                    #                                   aug_imgs]
                    #         # flipped GT
                    #         # augmented_GT_bboxes = self.aug_bboxes(x, img_metas, selected_gt_novel_bboxes)
                    #         # non-flipped GT
                    #         augmented_GT_bboxes = selected_gt_novel_bboxes
                    #
                    #         # flipped predicted bboxes
                    #         # augmented_bboxes = self.aug_bboxes(x, img_metas, novel_proposals_coors)
                    #         # non-flipped predicted bboxes
                    #         augmented_bboxes = novel_proposals_coors
                    #
                    #         augmented_sampling_results = []
                    #         novel_gt_labels = []
                    #
                    #         # novel_gt_labels
                    #         for gt_label in gt_labels:
                    #             novel_gt_labels.append(
                    #                 torch.tensor([label for label in gt_label if label in range(15, 20)]).to(
                    #                     device="cuda"))
                    #
                    #         total_aug_iter = len(augmented_img_features)
                    #         # non-flipped
                    #         for j in range(num_imgs):
                    #             if novel_gt_labels[j].shape[0] != 0 and len(augmented_bboxes[j]) != 0:
                    #                 augmented_assign_result = bbox_assigner.assign(
                    #                     augmented_bboxes[j], augmented_GT_bboxes[j], None,
                    #                     novel_gt_labels[j])
                    #                 # 11/19 testing
                    #                 if novel_gt_labels[0].numel() and novel_gt_labels[1].numel():
                    #                     feats = [sett[j][None] for combined_feat in augmented_img_features for sett in
                    #                              combined_feat]
                    #                 else:
                    #                     feats = [sett for combined_feat in augmented_img_features for sett in
                    #                              combined_feat]
                    #                 ori_aug_assign_result = copy.copy(augmented_assign_result)
                    #                 # aug횟수만큼 sampling을 따로 따로 해줌 - feats가 달라지니까...
                    #                 for c in range(total_aug_iter):
                    #                     bf = feats[5 * c:5 * (c + 1)]
                    #                     if c != 0:
                    #                         augmented_assign_result = copy.copy(ori_aug_assign_result)
                    #                     augmented_sampling_result = bbox_sampler.sample(
                    #                         augmented_assign_result,
                    #                         augmented_bboxes[j],
                    #                         augmented_GT_bboxes[j],
                    #                         novel_gt_labels[j],
                    #                         feats=bf)
                    #                     augmented_sampling_results.append(augmented_sampling_result)
                    #             # else:
                    #             #     augmented_sampling_results.append(torch.tensor([]).to(device="cuda"))
                    #         if len(augmented_sampling_results) == total_aug_iter * 2:
                    #             final_aug_sampling_results = [
                    #                 [augmented_sampling_results[n], augmented_sampling_results[n + total_aug_iter]] for
                    #                 n in
                    #                 range(total_aug_iter)]
                    #         else:
                    #             final_aug_sampling_results = [
                    #                 [augmented_sampling_results[n], augmented_sampling_results[n]] for n in
                    #                 range(total_aug_iter)]
                    #         # print('hey2')
                    #
                    #         losses = self.get_loss_for_aug(i, augmented_img_features,
                    #                                        augmented_GT_bboxes, novel_gt_labels,
                    #                                        rcnn_train_cfg, losses, lw,
                    #                                        final_aug_sampling_results, empty, non_empty)
        return losses
    ...
    def aug_imgs(self, selected_gt_novel_bboxes, img, img_metas, num_augs):
        ...
        for k, selected_gt_novel_bbox in enumerate(selected_gt_novel_bboxes):
            if selected_gt_novel_bbox.shape[0] != 0:
                augmented_img.append(img_transforms(img[k], img_metas[k], num_augs))
            else:
